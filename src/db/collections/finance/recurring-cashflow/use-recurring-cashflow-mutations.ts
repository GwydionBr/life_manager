  ...
  const handleAddRecurringCashflow = useCallback(
    async (
      ...
      try {
        ...
        if (!result) {
          console.error(
            "Failed to create recurring cashflow:",
            newRecurringCashflow
          );
          if (showNotification) {
            showActionErrorNotification(
  ...
  const handleUpdateRecurringCashflow = useCallback(
    async (
      ...
      try {
        ...
        if (!result) {
          console.error("Failed to update recurring cashflow:", id, item);
          if (showNotification) {
            showActionErrorNotification(
              getLocalizedText(
    ...
    async (
      ...
      try {
        ...
        if (mode === DeleteRecurringCashFlowMode.keep_unlinked) {
          ...
          if (!result) {
            console.error("Failed to delete recurring cashflow:", ids);
            if (showNotification) {
              showActionErrorNotification(
                getLocalizedText(
          ...
          if (!singleCashflowResult) {
            console.error(
              "Failed to delete linked single cashflows:",
              singleCashflowsToDelete.map((cashflow) => cashflow.id)
            );
            if (showNotification) {
              showActionErrorNotification(
